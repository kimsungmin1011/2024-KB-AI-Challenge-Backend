...
import com.example.demo.dart.service.CompanyService;
...
public class CompanyController {
    ...
    @Autowired
    public CompanyController(CompanyService companyService, ExternalApiService externalApiService) {
    ...
    @GetMapping("/search")
    public List<Company> searchCompany(@RequestParam String name) {
    ...
    public String getFinancialData(@RequestParam String companyName) {
        List<Company> companies = companyService.searchCompanyByName(companyName);
        if (companies.isEmpty()) {
            return "회사를 찾을 수 없습니다";
        }
        Company company = companies.get(0); // 첫 번째 결과를 사용
    ...
    public String getFinancialDataByCode(@RequestParam String companyCode) {
